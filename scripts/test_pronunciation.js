...
const TEST_SAMPLES = [
  { text: 'מלח', wordNikud: 'מֶ-לַח.', label: 'Melach_Milel_Dash' },
  { text: 'מלח', wordNikud: 'מֶלַח.', label: 'Melach_Plain' },
  { text: 'וופל', wordNikud: 'וָ-פֶל.', label: 'Wafel_Milel_Dash' },
  { text: 'דג', wordNikud: 'דָּג.', label: 'Dag_Plain' },
  { text: 'חבל', wordNikud: 'חֶבֶל.', label: 'Chevel_Plain' },
  { text: 'יונה', wordNikud: 'יוֹ-נָה.', label: 'Yona_Dash' },
  { text: 'יונה', wordNikud: 'יוֹנָה.', label: 'Yona_Plain' },
];

async function generateTTS(text, filePath) {
  const fullText = `המילה היא - ${text}`;
  console.log(`Generating: "${fullText}"...`);
  ...
  return new Promise((resolve, reject) => {
    const postData = JSON.stringify({
      ...
      model_id: MODEL_ID,
      voice_settings: {
        stability: 0.35, // Lower stability for more natural prosody
